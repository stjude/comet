### Wrapper for core RMS survival, integrative (SAMPLER/H&E + methylation, see
### core_code/rms_survival_integrative_core.py). Same design as run_rms_survival_sampler.py
### — see that file's docstring — just swaps in the integrative core + both representations
### pickles.
###
### Usage:
###   python run_rms_survival_integrative.py --metadata ../data/RMS_survival_metadata.p \
...
###   python run_rms_survival_integrative.py ... --n-splits 5   # quick test
...
from rms_survival_integrative_core import classify_rms_survival_integrative  # noqa: E402
from safe_io import resolve_path, safe_pickle_load, safe_pickle_dump  # noqa: E402

TEMP_DIR=os.path.join(SCRIPT_DIR,'temp_rms_survival_integrative_csv')
...
def run_all_splits(metadata_file,splits_file,sampler_reps_file,methylation_reps_file,n_splits=None,params=None):
    ...
    for ni in range(N):
        ...
        results=classify_rms_survival_integrative(
            train_csv,test_csv,sampler_reps_file,methylation_reps_file,params=params,
        )
        for R in results:
            R['split']=ni
...
def main():
    ap=argparse.ArgumentParser(description='RMS survival wrapper (integrative: SAMPLER+methylation)')
    ...
    ap.add_argument('--out',default='rms_survival_integrative_results.p')
    ...
    all_results=run_all_splits(
        args.metadata,args.splits,args.sampler_reps,args.methylation_reps,n_splits=args.n_splits,
    )
