### NB MYCN amplification (binary) — WSI sampler-rep classification, thin wrapper around
### core_code/wsi_tumor_classification_core.py. Same design as
### tumor_classification/run_wsi_tumor_classification.py: reads a train/test SPLITS
### pickle (schema: split_i -> slides_train, slides_lab_train, slides_test, slides_lab_test),
### writes per-split train/test CSVs (columns: slide,label), hands them to the core
...
###   python run_nb_mycn_wsi_classification.py --sampler-reps ../data/univ2_20x_sample_reps.p --n-splits 5   # quick test
...
TEMP_DIR=os.path.join(SCRIPT_DIR,'temp_nb_mycn_wsi_csv')
...
def main():
    ap=argparse.ArgumentParser(description='NB MYCN amplification — WSI sampler-rep classification wrapper')
    ...
    ap.add_argument('--out',default='nb_mycn_wsi_results.p')
