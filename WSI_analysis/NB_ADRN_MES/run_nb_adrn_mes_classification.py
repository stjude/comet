### NB ADRN-vs-mesenchymal (MES) classification (binary: 0=ADRN, 1=MES) — WSI sampler-rep
### classification, thin wrapper around core_code/wsi_tumor_classification_core.py. Same
### design as tumor_classification/run_wsi_tumor_classification.py and
### NB_MYCN/run_nb_mycn_wsi_classification.py: reads a train/test SPLITS pickle (schema:
### split_i -> slides_train, slides_lab_train, slides_test, slides_lab_test), writes
### per-split train/test CSVs (columns: slide,label), hands them to the core classifier,
### and books-keeps the metrics across all splits/hyperparameters.
###
### The cohort itself (data/NB_ADRN_MES_slides.p: final slides + labels) is semi-supervised
### -- H&E-morphology seed slides grown via nearest-neighbor expansion, plus methylation-
### derived MES-probability labels for the rest -- see data/gen_nb_adrn_mes_labels.py.
###
### Usage:
...
DEFAULT_SPLITS=os.path.join(SCRIPT_DIR,'nb_adrn_mes_cv_splits.p')
TEMP_DIR=os.path.join(SCRIPT_DIR,'temp_nb_adrn_mes_wsi_csv')
...
def main():
    ap=argparse.ArgumentParser(description='NB ADRN-vs-MES — WSI sampler-rep classification wrapper')
    ap.add_argument('--splits',default=DEFAULT_SPLITS,help='NB ADRN/MES splits pickle')
    ap.add_argument('--sampler-reps',required=True,help='WSI sampler representations pickle')
    ap.add_argument('--out',default='nb_adrn_mes_wsi_results.p')
