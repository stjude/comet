### Wrapper for core integrative (WSI + methylation) tumor classification (see
### core_code/integrative_tumor_classification_core.py). Same design as
### run_wsi_tumor_classification.py / run_methylation_tumor_classification.py: reads a
### train/test SPLITS pickle (schema: split_i -> slides_train, slides_lab_train,
### slides_test, slides_lab_test), writes per-split train/test CSVs (columns: slide,label),
### hands them to the core classifier, and books-keeps the metrics across all
### splits/hyperparameters.
###
### Usage:
###   python run_integrative_tumor_classification.py --splits splits.p \
###     --sampler-reps ../data/univ2_20x_sample_reps.p \
###     --methylation-reps ../data/meth_top5000_sample_reps.p --out results.p
###   python run_integrative_tumor_classification.py --splits splits.p \
###     --sampler-reps ../data/univ2_20x_sample_reps.p \
###     --methylation-reps ../data/meth_top5000_sample_reps.p --n-splits 3   # quick test
...
from integrative_tumor_classification_core import classify_integrative  # noqa: E402
from safe_io import resolve_path, safe_pickle_load, safe_pickle_dump  # noqa: E402

TEMP_DIR=os.path.join(SCRIPT_DIR,'temp_integrative_tumor_class_csv')
...
def run_all_splits(splits_file,sampler_reps_file,methylation_reps_file,n_splits=None,params=None):
    ...
    for ni in range(N):
        ...
        results=classify_integrative(train_csv,test_csv,sampler_reps_file,methylation_reps_file,params=params)
        for R in results:
            R['split']=ni
...
def main():
    ap=argparse.ArgumentParser(description='Integrative (WSI+methylation) tumor classification wrapper')
    ap.add_argument('--splits',required=True,help='train/test splits pickle')
    ap.add_argument('--sampler-reps',required=True,help='WSI sampler representations pickle')
    ap.add_argument('--methylation-reps',required=True,help='methylation representations pickle')
    ap.add_argument('--out',default='integrative_tumor_class_results.p')
    ap.add_argument('--n-splits',type=int,default=None,help='limit number of splits (e.g. for a quick test)')
    args=ap.parse_args()

    all_results=run_all_splits(args.splits,args.sampler_reps,args.methylation_reps,n_splits=args.n_splits)
